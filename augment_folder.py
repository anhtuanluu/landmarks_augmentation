...
dataroot = 'caotoc_crop/images/'
path_annotations = glob.glob(dataroot + '*.json')
save_dir = 'caotoc_augment/'
...
for path_annotation in path_annotations:
    path_image = dataroot + os.path.basename(path_annotation)[:-4]+"jpg"
    print(path_image)
    ...
    def pannelplot(figID=0, dir_image=None, nrow_plot = 5,ncol_plot = 1, fignm="fig",save=True):        
        ...
        for x_train,y_train in generator.flow(xy,batch_size=1):
            ...
            if len(x_train) == 1:
                ...
                cv2.imwrite(save_dir + 'images/' + base_name[:-4] + '_' + str(count) + '.jpg', x_train[0])
                data = {
                    "version": "4.5.6",
                }
                data['flags'] = {}
                ...
                with open(save_dir + 'images/' + base_name[:-4] + '_' + str(count) + '.json', 'w') as outfile:
