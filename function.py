...
class ImageDataGenerator_landmarks(object):
    ...
    def _keep_only_valid_image(self,yimg):
        ...
        for irow in range(yimg.shape[0]):
            ...
            yobj = yimg[irow,:,:,self.loc_obj]
            y     = self._findindex_from_mask(ymask,yobj)
    ...
    def _findindex_from_mask(self,ymask,yobj):
        '''
        ymask : a mask of shape (height, width, 1)
        '''
        y_total = []
        for j in range(self.Nobject):
            ys = []
            ix, iy = np.where(yobj==j)
            if len(ix) == 0:
                return(None)
            y_total.append([ix,iy])

            # for i in range(self.Nlandmarks):
            #     for inx, iny in zip(ix, iy):
            #         if ymask[inx, iny]==i:
            #             # if len(ix) == 0:
            #             return(None)
    ...
    def get_ymask(self, img, xys):
        ...
        mask : A numpy array of size (height, width, channels). 
               ...
               ...
        '''
        yimg = np.zeros((img.shape[0],img.shape[1],2))
        yimg[:] = -1
        for i, xys_i in enumerate(xys):
            for iland, (ix,iy) in enumerate(xys_i):
                yimg[iy,ix] = iland, i
        
        self.Nlandmarks = len(xys[0]) # 5
        self.Nobject = len(xys) # num object
        self.loc_mask   = img.shape[2] # 3
        self.loc_obj = img.shape[2] + 1 #4
